...
<div class="panel-body">
	    ...
	    <div class="form-group">
	      ...
	      <div class="form-group">
	        ...
	        <select class="form-control" name="nama">
	        	<option>== Pilih ==</option>
	            <?php 
					$query = $koneksi->query("SELECT * FROM tb_anggota ORDER by nosis");
					
					while ($nama=$query->fetch_assoc()) {
						echo "<option value='$nama[nosis].$nama[nama]'>$nama[nosis] - $nama[nama]</option>";
					}
 ...
 // 	$dapatsiswa	= isset($_POST['nama']) ? $_POST['nama'] : "";
	...
	// $cek = $koneksi->query("SELECT * FROM tb_transaksi WHERE nosis=$siswa_id");
	// var_dump($cek);
...
if (isset($_POST['simpan'])) 
{
	...
	$siswa			= $pecah_siswa[1];
		...
		while ($hasil=$query->fetch_assoc()) {
			...
			$cek=$koneksi->query("SELECT * FROM tb_transaksi WHERE nosis=$id_siswa AND id_buku=$id_buku");
